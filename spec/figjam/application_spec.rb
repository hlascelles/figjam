...
describe Figjam::Application do
  before do
    allow_any_instance_of(described_class).to receive(:default_path).and_return("/path/to/app/config/application.yml")
    allow_any_instance_of(described_class).to receive(:default_environment).and_return("development")
  end
